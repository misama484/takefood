...
const PizzaOptions = ({pizza, nombre, descripcion, ingredientes}) => {
  ...
  const agregarIngrediente = (ingrediente) => {
    if (ingredientesPizza.includes(ingrediente)) {
      alert("ingrediente ya en la pizza")
      return
    }
    setIngredientesPizza([...ingredientesPizza, ingrediente])
  }
  const eliminarIngrediente = (ingrediente) => {
    let newIngredientes = ingredientesPizza.filter((ing) => ing !== ingrediente)
    setIngredientesPizza(newIngredientes)
  }
    ...
    <div className='w-[90vh] h-auto rounded-lg my-4 bg-gray-400 '>
      ...
      <div className=''>
        ...
        <div className='grid grid-cols-3 border '>
          {ingredientesPizza.map((ingrediente, index) => (
